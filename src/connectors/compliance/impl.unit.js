...
describe('compliance impl', function () {
    ...
    describe('POST templates', function () {
        test('obtain templates', async function () {
            const data = mock.getTemplates();
            const http = base.getSandbox().stub(request, 'run').resolves({
                statusCode: 200,
                body: {
                    data
            ...
            const result = await impl.getTemplates(['id', 'id']);
            result.should.have.size(1);
            result.data[0].version.should.equal('mock');
        ...
        test('returns empty array on unknown id', async function () {
            const http = base.getSandbox().stub(request, 'run').resolves({
                statusCode: 404,
                body: {},
                headers: {}
            ...
            await expect(impl.getTemplates(['id', 'id'])).resolves.toBeNull();
